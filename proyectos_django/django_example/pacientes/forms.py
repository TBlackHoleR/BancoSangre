...
from person.models import Donacion
from .models import Pacientes


class PacientsForm(forms.ModelForm):
    
    quantities = Donacion.objects.values_list('quantity', flat=True).distinct()   
    ...
    parish = forms.ModelChoiceField(
        label=_('¿Ha recibido una donacion anteriormente?:'), queryset=Parish.objects.all(),
        empty_label=_('Seleccione...'),
        widget=forms.Select(attrs={
            'class': 'form-control form-control-sm select2',
            'data-toggle': 'tooltip', 'disabled': 'true',
            'title': _('Seleccione Si/No'),
        })
    )
    ...
    cantidadS = forms.ModelChoiceField(
            label=_('Seleccione cantidad de sangre a donar:'), queryset=quantities,
            empty_label=_('Seleccione...'),
            widget=forms.Select(attrs={
                'class': 'form-control form-control-sm select2',
                'data-toggle': 'tooltip',
